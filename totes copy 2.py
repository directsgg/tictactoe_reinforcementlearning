        ...
        while True:
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and not self.isEnd:
                    if self.isComputerPlayer1:
                        # jugador 1
                        existWinner, invalidAction = self.playerTurn(self.p2)
                        # jugador 2
                        if not existWinner and not invalidAction:
                            self.playerTurn(self.p1)
                    else:
                        # jugador 1
                        existWinner, invalidAction = self.playerTurn(self.p1)
                        # jugador 2
                        if not existWinner and not invalidAction:
...
if __name__ == "__main__":
    stBoard = State()
    p1 = Agent(stBoard, "Humano2jaja", exp_rate=0)
    # p1.loadPolicy("policy_p1_extreme")
    p2 = Agent(stBoard, "Computadora", exp_rate=0)
